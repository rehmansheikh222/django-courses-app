#!/usr/bin/env python
"""
Package metadata for django_courses_app.
"""
import os
import re
import sys
...
def load_requirements(*requirements_paths):
    ...
    def check_name_consistent(package):
        ...
        #     raise Exception(
        #         f'Encountered both "{seen_spelling}" and "{
    ...
    def add_version_constraint_or_raise(current_line, current_requirements, add_if_not_present):
        ...
        if regex_match:
            ...
            #                         f' "{existing_version_constraints}" and "{
            #                             version_constraints}".'
            #                         f'Combine constraints into one location with {
            #                             package}'
...
VERSION = get_version('django_courses_app', '__init__.py')
...
setup(
    name='django-courses-app',
    version=VERSION,
    description="""One-line description for README and other doc files.""",
    long_description=README + '\n\n' + CHANGELOG,
    author='Abdul Rehman',
    author_email='abdul.rehman02@arbisoft.com',
    url='https://github.com/rehmansheikh222/django-courses-app',
    packages=find_packages(
        include=['django_courses_app', 'django_courses_app.*'],
        exclude=["*tests"],
    ),
    entry_points={
        "lms.djangoapp": [
            "django_courses_app = django_courses_app.apps:DjangoCoursesAppConfig",
        ],
        "cms.djangoapp": [
        ],
    },
